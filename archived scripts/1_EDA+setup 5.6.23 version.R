...
# run an initial random forest on subset of data to select 200 most important predictors 
## create subset of data - 10% of data
set.seed(212) # set seed
reg_split <- initial_split(reg_train, prop = 0.1, strata = y)
...
## Create list of 200 best predictors from initial rf
...
## list of vars for interactions (3 most important in rf that are not in PCA)
int_pred <- best_pred %>% 
...
recipe_main <- recipe(y ~ ., data = reg_train) %>%
  step_rm(all_predictors(), -best_pred) %>% # remove all but the best 200 predictors
  ...
  step_pca(pca_pred, threshold = .85) %>% # conduct PCA with highly correlated vars 
